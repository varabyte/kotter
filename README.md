![version: 1.1.1](https://img.shields.io/badge/kotter-v1.1.1-blue)
...
Fixed width columns are useful, but Kotter also provides functionality via fit- and star-sized columns.
...
A **star-sized column** will be resized dynamically based on how much space is remaining (more on this in a bit). If you
...
So if the target width is 10, and there is a fixed width column of size 4 and a single star-sized column, then the
star-sized column will be set to 6.

#### Column specs and Cols.fromStr

Earlier, we used the very simple `Cols` constructor that accepted only fixed widths. But for more control, you can
construct one using `Cols.Spec` instances directly:
